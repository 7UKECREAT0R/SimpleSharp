    ...
    [DllImport("kernel32.dll", SetLastError = true)]
        ...
        public static void WriteAt(int _x, int _y, object _text)
        {
            ...
            for(int ch = 0; ch < text.Length; ch++)
            {
                ...
                membuffer[ch + oset].Attributes = (short)15; // white
            }
            return;
        }
        public static void RenderWLQUEUE()
        {
            ...
            LowLevelCharInfo[] ptrb = membuffer;
            byte[] ascii = new byte[ptrb.Length];
            int i = -1;
            foreach(LowLevelCharInfo ll in ptrb)
            {
                i++;
                ascii[i] = ll.Char.AsciiChar;
            }
            PInvokeClear();
            WriteConsoleOutput(cHandle, ptrb, sz,
